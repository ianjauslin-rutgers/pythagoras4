...
lemma neq_of_para {a b : point} {L M : line}
    ...
    (pLM: para L M) :
    a ≠ b := fun ab => by
  have := offline_of_para aL pLM
  rw [ab] at this; exact this bM
...
lemma not_online_of_triangle {a b c : point} {L M : line}
    ...
    (hn: ¬ online a M)
    (bc: b ≠ c) :
    ¬ online c L := fun cL => by
  rw [line_unique_of_pts bc bL cL bM cM] at aL; exact hn aL
...
theorem eq_area_of_eq_base_samevertex (a : point) {b c e f : point} {L : line}
  ...
  · perm [(area_zero_iff_online bc bL cL).mpr aL]; rw [this]
    perm [(area_zero_iff_online ef eL fL).mpr aL]; rw [this]
...
theorem para_implies_eq_area_of_same_base {a b c d : point} {L M : line}
  ...
  exact eq_area_of_eq_base aM bL cL dM bL cL pLM rfl
...
lemma tri_sum_contra {b c d e: point} {O : line}
  ...
  have ar2 := eq_area_of_eq_base aN bL cL eN bL cL pNL rfl
  rw [ar] at ar2
