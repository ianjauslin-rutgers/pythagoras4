...
/-begin
    ...
    cases this,
    exact lt_area_of_lt_base__sameedge_Bbfc a hbL hcL hfL this h_b_ne_c h_b_ne_f h_c_ne_f h_a_nonline_L hlen,
...
/-begin
  ...
  have ca : c ≠ a := (ne_13_of_B Baec).symm,
  ...
  have hndO: ¬ online d O,
  { by_contradiction contra,
    exact NO (line_unique_of_pts ad haN hdN haO contra), },
  have hnaL:= λ haL, hneN (by rwa line_unique_of_pts ad haL hdL haN hdN at heL),
  have hnbL:= λ hbL, hneN (by rwa line_unique_of_pts bd hbL hdL hbN hdN at heL),
  have hncL:= λ hcL, 
    (by rwa ← line_unique_of_pts ce hcO heO hcL heL at hnaL : ¬ online a O) haO,
  have bc : b ≠ c,
  { by_contradiction contra,
  ...
  have Bbac : ¬ B b a c,
  { by_contradiction contra,
    have := online_3_of_B contra hbN haN,
    have := line_unique_of_pts ca.symm haO hcO haN this,
    exact NO this.symm, },
    have hneP : ¬ online e P,
    { by_contradiction contra,
      exact PO (line_unique_of_pts ce hcP contra hcO heO), },

  have hbdad_by_prop_vi_i : proportion (length b d) (length a d) (area e b d) (area e a d) := by
  ...
  have len_ad_neq_0 : length a d ≠ 0 :=
    begin
      by_contradiction,
      have h1 : a = d := by exact (length_eq_zero_iff.1 h),
      exact ad h1,
    end,
  have len_ae_neq_0 : length a e ≠ 0 :=
   
    begin
      by_contradiction,
      have h1 : a = e := by exact (length_eq_zero_iff.1 h),
      exact ae h1,
    end,
  have area_ade_neq_0 : area a d e ≠ 0 :=
   
    begin
      by_contradiction,
      have h1 := (area_zero_iff_online ad haN hdN).1 h,
      exact hneN h1
    end,
  ...
  have ratio_bd_ad := eq_ratio_of_proportion len_ad_neq_0 area_ead_neq_0 hbdad_by_prop_vi_i,
  have ratio_ce_ae := eq_ratio_of_proportion len_ae_neq_0 area_dae_neq_0 hceae_by_prop_vi_i,
  have ratio_iff : length b d / length a d = length c e / length a e ↔ area e b d / area e a d = area d c e / area d a e := by rwa [ratio_bd_ad, ratio_ce_ae],
  have proportion_lhs : proportion (length b d) (length a d) (length c e) (length a e) ↔ area e b d / area e a d = area d c e / area d a e :=
  ...
  have area_bde_eq_cde : proportion (length b d) (length a d) (length c e) (length a e) ↔ area b d e = area c d e :=
  ...
  intro pLM,
  rw area_bde_eq_cde,
  apply para_implies_eq_area_of_same_base hbM hdL heL hcM pLM,  end-/
...
/-begin
    ...
    exact tri_def (false.rec (colinear d e f) (this rfl)),
    ...
    refine this tri_abc ang_a_eq_d.symm ang_b_eq_e.symm leq.symm a_ne_c d_ne_f a_ne_b d_ne_e e_ne_f b_ne_c DF hDF DE hDE EF hEF f_nonline_DE tri_def e_nonline_DF _ _,
...
/-begin
    ...
    have hag_bac : angle h a g = angle b a c,
...
/-begin
    ...
    { wlog df_ne_ac : length d f = length a c,
      ...
      have leq : length d e = length a b ,
      { cases dfde_ne_acab,
        exfalso,
