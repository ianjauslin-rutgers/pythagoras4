...
/-- technical lemma that really shouldn't be here, but hey... -/
lemma ge2_of_n1_n0 {n : ℕ} (h0: n ≠ 0) (h1: n ≠ 1) : n ≥ 2
:= by exact ge_iff_le.mpr ((Nat.two_le_iff n).mpr ⟨ h0,h1 ⟩)


/-- technical lemma that really shouldn't be here, but hey... -/
lemma gt1_of_n1_n0 {n : ℕ} (h0: n ≠ 0) (h1: n ≠ 1) : n > 1
...
/-- from segment of length l, construct a new segment of length n*l
    based on I.3 -/
lemma rescale_length {a b : point} {L : line} 
(n : ℕ)
(haL: online a L)
(hbL: online b L)
...
:= by
  ...
  | succ n hn =>
    ...
    by_cases hn1 : n = 1
    {
      obtain ⟨ e, he ⟩ := same_length_B_of_ne_four nab nab
      use e
      constructor
      exact online_3_of_B he.1 haL hbL
      rw [← length_sum_of_B he.1, hn1, he.2]
      constructor
      ring
      simp
      intro
    ...
    have nad : a ≠ d
    {
      ...
      rw [hd.2.1]
      simp only [Nat.pos_of_ne_zero hn0, zero_lt_mul_left, Nat.cast_pos]
      exact len_pos_of_nq nab
    }

    rw [(Eq.symm (Ne.def a b))] at nab
    obtain ⟨ e, he ⟩ := same_length_B_of_ne_four nad nab
    use e
    ...
    have : n ≥ 2 ∧ a ≠ b := ⟨ ge2_of_n1_n0 hn0 hn1, nab ⟩
    intros
    exact B124_of_B134_B123 he.1 (hd.2.2 this)
...
:= by 
    ...
    | succ n hn =>
    {
      ...
      by_cases h_n_ne_1 : n = 1
      {
        ...
        simp only [Nat.cast_succ] at hlen
        rw [add_mul] at hlen
        simp at hlen
        exact hlen.symm
      }

      -- constructor off n+1'st bit
      simp only [Nat.cast_succ, one_mul]
      rw [add_mul]
      ...
      have h_B_bed : B b e d := by
      {
        ...
        | inr hb =>
          ...
          have := B124_of_B134_B123 hb (B_symm h_B_bce)
          exact h1 this
...
:= by
    ...
    cases hBs with
    | inl hl =>
    exact rescale_triangle_of_base__B a hbL hcL hdL hlen hl h_a_nonline_L
    | inr hr =>
      exfalso
      exact hB hr
...
:= by
    ...
    have h_b_ne_d: b ≠ d
    {
      ...
      | inl hl =>
        exact h_n_ne_0 hl
      | inr hr =>
        exact h_b_ne_c (length_eq_zero_iff.mp hr)
    }
...
:= by
  ...
  cases this with
  | inl ha =>
    exact lt_area_of_lt_base__sameedge_Bbfc a hbL hcL hfL ha h_b_ne_c h_b_ne_f h_c_ne_f h_a_nonline_L hlen
...
:= by 
  ...
  by_cases hB: B f b c
  {
    ...
    have lbe : length b e = length b f
    {
...
:= by
  ...
  have hlen_ef_bh : length e f = length b h
  {
...
:= by
  ...
  rw [length_symm d b, length_symm e c]
  simp only [add_div]
...
:= by
  rw [colinear_symm2, colinear_symm2]
...
: para BC HG
:= by
    have hbc_abc : angle h b c = angle a b c
    have Z:= b_ne_h.symm
    have Z1:= a_ne_b
    have Z2:= b_ne_c.symm
    have Z3:= b_ne_c.symm
    ...
    sorry
    -- TODO: fix refine
    -- refine angle_extension b_ne_h.symm a_ne_b b_ne_c.symm b_ne_c.symm hbAB hhAB haAB hbBC hcBC hcBC (not_B_of_B (B_symm hB))
    -- by_contra contra
    -- have := ne_13_of_B contra
    -- exact b_ne_h (false.rec (b = h) (this rfl))
    ...
    -- rw [angle_symm, this.symm, angle_symm, an]
    -- exact hbc_abc.symm
    -- swap
...
(tri_abc : ¬ colinear a b c)
(tri_def : ¬ colinear d e f) 
(ang_a_eq_d : angle b a c = angle e d f)
(ang_b_eq_e : angle a b c = angle d e f)
...
:= by
  ...
  rw [colinear_symm1, colinear_symm2] at tri_abc
  have b_nonline_AC := not_online_of_not_colinear hAC.1 hAC.2 tri_abc

  by_contra contra

  rw [(Ne.def (length d e) (length a b)).symm, ne_iff_lt_or_gt] at contra
  ...
  rw [hac_bac.symm, angle_symm, @angle_symm i e d f] at ang_a_eq_d
  ...
  have tri_dfe := tri_def
  rw [colinear_symm1, colinear_symm2] at tri_dfe
  ...
  have e_nonline_DF := not_online_of_not_colinear hDF.1 hDF.2 tri_dfe
  sorry
  -- TODO: wlog seems to be causing problems with variable names
  -- refine this tri_acb tri_def ang_a_eq_d.symm ang_b_eq_e.symm leq.symm a_ne_c d_ne_f a_ne_b d_ne_e e_ne_f b_ne_c DF hDF DE hDE EF hEF f_nonline_DE tri_def e_nonline_DF _ _,
...
:= by
  ...
  sorry
  -- there seems to be a problem with cases naming
  -- cases contra

  -- obtain ⟨h, hh⟩ := same_length_B_of_ne_ge a_ne_b _
  ...
  -- rw [colinear_symm1, colinear_symm2] at tri_def
  -- rw [angle_symm, @angle_symm i e d f] at ang_a_eq_d
  -- have := length_eq_of_length_eq tri_abc tri_def ang_a_eq_d _ contra.symm
  -- linarith

  -- rw [angle_symm, @angle_symm i f d e] at ang_a_eq_d
  -- rw [angle_symm, @angle_symm i d e f] at ang_b_eq_e
...
theorem similar_of_AA {a b c d e f : point}
(tri_abc : ¬ colinear a b c)
(tri_def : ¬ colinear d e f)
(ang_a_eq_d : angle b a c = angle e d f)
(ang_b_eq_e : angle a b c = angle d e f)
...
:= by
  ...
  { wlog df_ne_ac : length d f = length a c
    ...
    rw [angle_symm, @angle_symm i e d f] at ang_a_eq_d
    sorry
    -- TODO: fix the type of dfde_ne_acab
    -- refine this tri_abc tri_def ang_a_eq_d _ d_ne_e a_ne_b d_ne_f a_ne_c b_ne_c.symm e_ne_f.symm (Or.swap dfde_ne_acab) leq
  ...
  sorry
  -- TODO: fix not_or_distrib 
  -- rw [not_or_distrib] at dfde_ne_acab
  ...
  --   rw [angle_symm] at this
  --   rw [this, angle_symm]
  -- rw [hag_bac.symm, angle_symm, @angle_symm i e d f] at ang_a_eq_d
  -- have ang_b_eq_h := sas hg.2 hh.2 ang_a_eq_d
  -- rw [ang_b_eq_e.symm] at ang_b_eq_h
